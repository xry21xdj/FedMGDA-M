...
def get_imagenet():
    ...
    dataset_path =  os.path.join("data", "imagenet", "raw_data")
    ...
    # Load the dataset, this is important
    train_dataset = ImageFolder(root=os.path.join(dataset_path, 'tiny-imagenet-200/train'), transform=transform)
    val_dataset = ImageFolder(root=os.path.join(dataset_path, 'tiny-imagenet-200/val'), transform=transform)
    test_dataset = ImageFolder(root=os.path.join(dataset_path, 'tiny-imagenet-200/test'), transform=transform)
    dataset = ConcatDataset([train_dataset, val_dataset, test_dataset])

    imagenet_dataset = dataset#ImageFolder(dataset_path,transform=transform)
