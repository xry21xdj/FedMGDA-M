...
# -*- coding: utf-8 -*-
"""
Created on Mon Aug 22 20:12:33 2022
...
def get_learner(
    ...
    if name == "synthetic":
        ...
        model = get_vgg16(n_classes=10).to(device)
        is_binary_classification = False
    elif name == "cifar100":
        criterion = nn.CrossEntropyLoss(reduction="none").to(device)
        metric = accuracy
        ...
        #model = get_mobilenet(n_classes=10).to(device)
        # model = get_resnet18(n_classes=10).to(device)
        model = get_resnet18(n_classes=200).to(device)
    ...
    optimizer =\
        get_optimizer(# get_optimizer defined in utils->optim.py
...
def get_client(
    ...
    if client_type == "mixture":
        ...
        return Client( #defined in client
            learners_ensemble=learners_ensemble,
            train_iterator=train_iterator,
            val_iterator=val_iterator,
            test_iterator=test_iterator,
...
def get_aggregator(
        ...
        data_size,
        attack_manner,
        scale, #byzatine attack variable
        mean, #byzantine attack variable
        variance, #byzantine attack variable
    ...
    if aggregator_type == "no_communication":
        return NoCommunicationAggregator(
            ...
            beta=beta,
            K=K,
            buffed_clients=buffed_clients,
            poisoned_num=poisoned_num,
            poisoned_gap=poisoned_gap,
            grad_bound=grad_bound,
            epsilon=epsilon,
            delta=delta,
            data_size=data_size,
            attack_manner=attack_manner,
            scale=scale, #byzatine attack variable
            mean=mean, #byzantine attack variable
            variance=variance, #byzantine attack variable
            test_clients=test_clients,
            sampling_rate=sampling_rate,
            local_E=local_E,
            verbose=verbose,
            add_noise=add_noise,
            poisoned_flag=poisoned_flag,
            seed=seed
        )
