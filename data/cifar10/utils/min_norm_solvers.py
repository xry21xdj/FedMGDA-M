...
class MinNormSolver:
    ...
    def find_min_norm_element(vecs):
        ...
        while iter_count < MinNormSolver.MAX_ITER:
            ...
            #print('gamma=',nc, 'newpoint=',new_point)
            new_sol_vec = nc*sol_vec + (1-nc)*new_point
            change = new_sol_vec - sol_vec
            #print('change_sum=',np.sum(np.abs(change)))
            iter_count+=1
            if np.sum(np.abs(change)) < MinNormSolver.STOP_CRIT:
                print('iteration for weight calculation=',iter_count,
                      'sum differ from Avg=',np.sum(np.abs(sol_vec-avg)))
                return sol_vec, nd
            sol_vec = new_sol_vec
        return sol_vec, nd
...
def parameter_normalizers(param, normalization_type):
    if normalization_type == 'l2':
        pn = np.sqrt(np.sum([pr.pow(2).sum().data.cpu() for pr in param]))
    return pn
